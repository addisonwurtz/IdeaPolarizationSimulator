...
from IdeaPolarizationSim.simulation_classes import NewsItem, User, GraphData, SocialNetwork, UserNotInfectiousError
...
class TestGraph(unittest.TestCase):
    ...
    def test_increase_connection_strength(self):
        ...
        edge_weight = {(1, 2): 0.5}
        test_graph = GraphData([user1, user2], edge_weight, update_rate=0.1)
    ...
    def test_get_edge_string(self):
        ...
        self.assertEqual((1, 2), GraphData.get_edge(user1, user2))
        self.assertEqual((1, 2), GraphData.get_edge(user2, user1))

    def test_get_edge_string_raises_value_error_when_nodes_are_the_same(self):
        user = User(1, 1.2, [])
        with self.assertRaises(ValueError):
            GraphData.get_edge(user, user)
    ...
    def test_get_edge_weight_for_valid_edge(self):
        ...
        test_graph = GraphData([user1, user2], edge_weight)
